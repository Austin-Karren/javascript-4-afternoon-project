...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function(num){
    let aveRating = (this.rating + num) /2;
    return aveRating;
  }
  ...
  User.prototype.addSavedPost = function(id, title, rating){this.savedPosts.push({id, title, rating})};
  ...
  User.prototype.removeSavedPost = function(num){this.savedPosts = this.savedPosts.filter(e => e.id !== num)}
  ...
  User.prototype.changePostRating = function(id, num){
    let foundObj = this.savedPosts.find(e => e.id === id);
    foundObj.rating = num;
  }
